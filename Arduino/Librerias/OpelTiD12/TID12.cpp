...
TID12::TID12(byte sda, byte scl, byte mrq) {
...
void TID12::space1_init(byte start, byte stop) {
	_space1start = start;
	_space1stop = stop;
}

void TID12::space2_init(byte start, byte stop) {
	_space2start = start;
	_space2stop = stop;
}

void TID12::space3_init(byte start, byte stop) {
	_space3start = start;
	_space3stop = stop;
}
void TID12::tid_byte(byte data) {
  pinMode(_scl, OUTPUT);
...
void TID12::tid_data(byte data) {      //bytes of data
...
void TID12::reset_pos(byte spaceid) {
	switch (spaceid) {
		case 1:
			_incr1 = 0;
			_ad1 = 0;
...
void TID12::clear_space(byte spaceid) {
	byte start;
	byte stop;
	switch (spaceid) {
		case 1:
...
void TID12::symbol_assign (byte symbo, byte RW) {
...
void TID12::display_symbol(byte symbo) {
	symbol_assign(symbo,1);
	cycle();
}

void TID12::clear_symbol(byte symbo) {
	symbol_assign(symbo,0);
	cycle();
}

void TID12::clear_all_symbols() {
	for (int i=1; i <= 11; i++) {
		symbol_assign(i,0);
	};
	cycle();
...
void TID12::display_message(String message, byte speed, byte spaceid) {
	...
	if ((millis() - timestamp) >= upd_delay) {
		...
		//Parte Modificada sin Arduino.h
		if (stop<(start-1+message.length())) {
			stop_temp = stop;
		}else {
			stop_temp = (start-1+message.length());
		}


		for (int i=start; i<=stop_temp ; i++) {
...
void TID12::bargraph(byte level) {
